  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-page)' }}>
      <NavBar
        ...
        badge={myEntry ? { value: parseFloat(myEntry.total_points || 0).toFixed(0), label: `Rank #${myEntry.overall_rank}`, color: 'var(--gold)' } : null}
      />
    ...
    <div>
      {Object.entries(grouped).map(([date, dayMatches]) => (
        <div key={date} style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 11, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 10, fontWeight: 700, fontFamily: 'var(--font-display)' }}>
          ...
          {dayMatches.map(m => {
            ...
            return (
              <div key={m.id} onClick={() => isDone && onSelectMatch(m)} style={{
                background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--card-radius-sm)',
                padding: '14px 18px', marginBottom: 10, cursor: isDone ? 'pointer' : 'default',
                transition: 'border-color 0.15s',
                borderColor: isLive ? 'var(--red)' : 'var(--border)'
              }}
                onMouseOver={e => isDone && (e.currentTarget.style.borderColor = 'var(--gold)')}
                onMouseOut={e => (e.currentTarget.style.borderColor = isLive ? 'var(--red)' : 'var(--border)')}
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  <span style={{ fontSize: 11, color: 'var(--text-faint)', fontFamily: 'var(--font-display)' }}>M{m.match_number} {m.match_type ? `· ${m.match_type.toUpperCase()}` : ''}</span>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    {isLive && <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 'var(--btn-radius)', background: 'rgba(239,68,68,0.15)', color: 'var(--red)', fontWeight: 700 }}>🔴 LIVE</span>}
                    {!isLive && <Badge status={m.status} />}
                    {isDone && <span style={{ fontSize: 11, color: 'var(--gold)' }}>Tap for scorecard →</span>}
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                  <div style={{ flex: 1, textAlign: 'left' }}>
                    <div style={{ fontWeight: winner?.id === m.team_a_id ? 700 : 400, fontSize: 14, color: winner?.id === m.team_a_id ? 'var(--accent)' : 'var(--text-primary)' }}>
                      {tA.short_name || tA.name || 'TBA'}
                      {winner?.id === m.team_a_id && ' 🏆'}
                    </div>
                    <div style={{ fontSize: 11, color: 'var(--text-faint)' }}>{tA.name}</div>
                  </div>
                  <div style={{ color: 'var(--text-faint)', fontSize: 13, fontWeight: 700, flexShrink: 0 }}>vs</div>
                  <div style={{ flex: 1, textAlign: 'right' }}>
                    <div style={{ fontWeight: winner?.id === m.team_b_id ? 700 : 400, fontSize: 14, color: winner?.id === m.team_b_id ? 'var(--accent)' : 'var(--text-primary)' }}>
                      {tB.short_name || tB.name || 'TBA'}
                      {winner?.id === m.team_b_id && ' 🏆'}
                    </div>
                    <div style={{ fontSize: 11, color: 'var(--text-faint)' }}>{tB.name}</div>
                  </div>
                </div>
                {m.result_description && (
                  <div style={{ marginTop: 10, fontSize: 12, color: 'var(--text-dim)', borderTop: '1px solid var(--border)', paddingTop: 8 }}>
                    {m.result_description}
                  </div>
                )}
                {m.match_time && !isDone && (
                  <div style={{ marginTop: 8, fontSize: 11, color: 'var(--text-faint)' }}>🕐 {m.match_time}</div>
                )}
              </div>
            )
          })}
...
function ScorecardView({ match, scorecard, teams, onBack }) {
  ...
  return (
    <div>
      <button onClick={onBack} style={{ background: 'var(--bg-card-alt)', border: '1px solid var(--border)', color: 'var(--text-dim)', padding: '8px 14px', borderRadius: 'var(--btn-radius)', fontSize: 13, marginBottom: 16 }}>
        ← Back to Fixtures
      </button>
      ...
      <Card style={{ marginBottom: 16 }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ textAlign: 'center', marginBottom: 12 }}>
            <span style={{ fontSize: 11, color: 'var(--text-dim)', fontFamily: 'var(--font-display)', textTransform: 'uppercase' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ textAlign: 'center', flex: 1 }}>
              ...
              {winner?.id === match.team_a_id && <div style={{ fontSize: 11, color: 'var(--accent)', marginTop: 2 }}>WINNER 🏆</div>}
            </div>
            <div style={{ color: 'var(--text-faint)', fontWeight: 700 }}>vs</div>
            <div style={{ textAlign: 'center', flex: 1 }}>
              <div style={{ fontWeight: 700, fontSize: 15 }}>{tB.short_name || tB.name}</div>
              {winner?.id === match.team_b_id && <div style={{ fontSize: 11, color: 'var(--accent)', marginTop: 2 }}>WINNER 🏆</div>}
            </div>
          </div>
          {match.result_description && <div style={{ textAlign: 'center', color: 'var(--text-dim)', fontSize: 13, marginTop: 10 }}>{match.result_description}</div>}
        </div>
      </Card>

      {!scorecard ? (
        <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-dim)' }}>Loading scorecard…</div>
      ) : (
        <>
          {/* Batting */}
          {scorecard.batting.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              ...
              <Card>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 40px 40px 40px 50px', gap: 6, padding: '8px 14px', background: 'var(--bg-card-alt)', fontSize: 11, color: 'var(--text-dim)', textTransform: 'uppercase' }}>
                ...
                {scorecard.batting.map((b, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 40px 40px 40px 50px', gap: 6, padding: '9px 14px', borderTop: '1px solid var(--border)', alignItems: 'center' }}>
                    ...
                    <div style={{ textAlign: 'right', fontWeight: 700, fontFamily: 'var(--font-display)' }}>{b.runs}</div>
                    <div style={{ textAlign: 'right', color: 'var(--text-dim)', fontFamily: 'var(--font-display)', fontSize: 12 }}>{b.balls_faced}</div>
                    <div style={{ textAlign: 'right', color: 'var(--text-dim)', fontFamily: 'var(--font-display)', fontSize: 12 }}>{b.fours || 0}</div>
                    <div style={{ textAlign: 'right', color: 'var(--text-dim)', fontFamily: 'var(--font-display)', fontSize: 12 }}>{b.sixes || 0}</div>
                  </div>
                ))}
              </Card>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <Card>
                ...
                {scorecard.bowling.map((b, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 40px 40px 40px 50px', gap: 6, padding: '9px 14px', borderTop: '1px solid var(--border)', alignItems: 'center' }}>
                    <div style={{ fontSize: 13, fontWeight: 500 }}>{b.local_players?.name || b.player_name}</div>
                    <div style={{ textAlign: 'right', fontFamily: 'var(--font-display)', fontSize: 12, color: 'var(--text-dim)' }}>{b.overs}</div>
                    <div style={{ textAlign: 'right', fontWeight: 700, color: 'var(--accent)', fontFamily: 'var(--font-display)' }}>{b.wickets}</div>
                    <div style={{ textAlign: 'right', fontFamily: 'var(--font-display)', fontSize: 12, color: 'var(--text-dim)' }}>{b.runs_conceded}</div>
                    <div style={{ textAlign: 'right', fontFamily: 'var(--font-display)', fontSize: 12, color: 'var(--text-dim)' }}>{b.economy_rate || '—'}</div>
                  </div>
                ))}
              </Card>
            </div>
...
function PointsTableTab({ pointsTable }) {
  ...
  const qualColors = { qualified: 'var(--accent)', eliminated: 'var(--red)', in_contention: 'var(--gold)' }
    ...
    <div>
      {Object.entries(groups).map(([group, rows]) => (
        <div key={group} style={{ marginBottom: 24 }}>
          <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 12, color: 'var(--gold)' }}>Group {group}</div>
          <Card>
            <div style={{ display: 'grid', gridTemplateColumns: '24px 1fr 30px 30px 30px 30px 50px 50px', gap: 6, padding: '10px 14px', background: 'var(--bg-card-alt)', fontSize: 10, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '0.3px' }}>
            ...
            {rows.map((r, i) => (
              <div key={r.id} style={{ display: 'grid', gridTemplateColumns: '24px 1fr 30px 30px 30px 30px 50px 50px', gap: 6, padding: '11px 14px', borderTop: '1px solid var(--border)', alignItems: 'center', background: r.qualification_status === 'qualified' ? 'rgba(34,197,94,0.04)' : 'transparent' }}>
                <div style={{ fontSize: 12, color: qualColors[r.qualification_status] || 'var(--text-faint)', textAlign: 'center' }}>
                ...
                <div style={{ textAlign: 'right', fontFamily: 'var(--font-display)', fontSize: 12, color: parseFloat(r.nrr || 0) >= 0 ? 'var(--accent)' : 'var(--red)' }}>
                  {parseFloat(r.nrr || 0).toFixed(3)}
                </div>
                <div style={{ textAlign: 'right', fontWeight: 700, fontFamily: 'var(--font-display)', color: 'var(--gold)' }}>{r.points}</div>
              </div>
            ))}
          </Card>
          <div style={{ display: 'flex', gap: 16, marginTop: 8 }}>
            <span style={{ fontSize: 11, color: 'var(--text-faint)' }}>✅ Qualified</span>
            <span style={{ fontSize: 11, color: 'var(--text-faint)' }}>🟡 In Contention</span>
            <span style={{ fontSize: 11, color: 'var(--text-faint)' }}>❌ Eliminated</span>
          </div>
        </div>
      ))}
    ...
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
        {['all', 'BAT', 'BOWL', 'AR', 'WK'].map(r => (
          <button key={r} onClick={() => setFilterRole(r)} style={{ padding: '7px 14px', borderRadius: 20, border: '1px solid', borderColor: filterRole === r ? 'var(--gold)' : 'var(--border)', background: filterRole === r ? 'rgba(245,158,11,0.15)' : 'var(--bg-card)', color: filterRole === r ? 'var(--gold)' : 'var(--text-dim)', fontSize: 12 }}>
            {r}
          </button>
        ))}
        <div style={{ flex: 1 }} />
        <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text-primary)', padding: '7px 10px', borderRadius: 'var(--btn-radius)', fontSize: 12 }}>
      ...
      <Card>
        <div style={{ display: 'grid', gridTemplateColumns: '32px 36px 1fr 55px 45px 45px', gap: 6, padding: '10px 14px', background: 'var(--bg-card-alt)', fontSize: 11, color: 'var(--text-dim)', textTransform: 'uppercase' }}>
        ...
        {sorted.map((p, i) => (
          <div key={p.id || i} style={{ display: 'grid', gridTemplateColumns: '32px 36px 1fr 55px 45px 45px', gap: 6, padding: '10px 14px', borderTop: '1px solid var(--border)', alignItems: 'center' }}>
            ...
            <div style={{ textAlign: 'right', fontWeight: 700, color: 'var(--gold)', fontFamily: 'var(--font-display)' }}>{parseFloat(p.fantasy_points || 0).toFixed(0)}</div>
...
function FantasyTab({ leaderboard, userId, myEntry }) {
  return (
    <div>
      ...
      <Card>
        <div style={{ display: 'grid', gridTemplateColumns: '48px 40px 1fr 65px 65px 80px', gap: 6, padding: '10px 14px', background: 'var(--bg-card-alt)', fontSize: 11, color: 'var(--text-dim)', textTransform: 'uppercase' }}>
        ...
        {leaderboard.length === 0
          ...
          : leaderboard.map((row, i) => {
            ...
            return (
              <div key={row.user_id} style={{ display: 'grid', gridTemplateColumns: '48px 40px 1fr 65px 65px 80px', gap: 6, padding: '11px 14px', borderTop: '1px solid var(--border)', background: isMe ? 'rgba(154,224,0,0.04)' : 'transparent', alignItems: 'center' }}>
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: isMe ? 700 : 500, color: isMe ? 'var(--gold)' : 'var(--text-primary)' }}>
                    {row.full_name || 'User'}
                    {isMe && <span style={{ fontSize: 10, background: 'var(--gold)', color: '#0f0f0f', padding: '1px 5px', borderRadius: 4, marginLeft: 5, fontWeight: 700 }}>YOU</span>}
                  </div>
                  {row.team_name && <div style={{ fontSize: 11, color: 'var(--text-faint)' }}>{row.team_name}</div>}
                </div>
                <span style={{ textAlign: 'right', fontSize: 12, color: 'var(--text-dim)', fontFamily: 'var(--font-display)' }}>{parseFloat(row.group_a_points || 0).toFixed(0)}</span>
                <span style={{ textAlign: 'right', fontSize: 12, color: 'var(--text-dim)', fontFamily: 'var(--font-display)' }}>{parseFloat(row.group_b_points || 0).toFixed(0)}</span>
                <span style={{ textAlign: 'right', fontWeight: 700, color: 'var(--accent)', fontFamily: 'var(--font-display)' }}>{parseFloat(row.total_points || 0).toLocaleString()}</span>
              </div>
            )
          })
        }
...
function StatBox({ label, value, color }) {
  return (
    <div>
      ...
      <div style={{ fontSize: 17, fontWeight: 700, color: color || 'var(--text-primary)', fontFamily: 'var(--font-display)' }}>{value}</div>
    </div>
  )
}
